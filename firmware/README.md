...
The same firmware and serial protocol support these 8×8 targets:
...
| `wemos_d1_mini_ws2812b_64` | Wemos D1 mini (ESP8266) with external WS2812B-64 8×8 matrix | D4 (GPIO 2) | GRB, serpentine |

Install PlatformIO only on the development/flashing machine. Build all targets with:

```sh
pio run -e waveshare_esp32_s3_matrix -e esp32_wroom_32d_ws2812b_64 -e wemos_d1_mini_ws2812b_64
```
...
## ESP32-WROOM-32D / Wemos D1 mini matrix wiring

The WROOM uses GPIO 16; the original Wemos D1 mini uses its `D4` header pin (GPIO 2):

| ESP32 / supply | WS2812B-64 |
| --- | --- |
| GPIO 16 (WROOM) or D4 / GPIO 2 (Wemos) through a 330–470 Ω resistor | `DIN` |
| Ground | `GND` |
...
Most WS2812B-64 panels use GRB pixels in a serpentine row layout. If the `FIVE_THREE` pattern is mirrored or rotated on a particular panel, adjust `MATRIX_SERPENTINE` and `MATRIX_ROTATION` in the appropriate external-matrix environment. Solid presence colors are unaffected by matrix orientation.
...
Run the parser unit tests on a connected board with `pio test -e waveshare_esp32_s3_matrix`, `pio test -e esp32_wroom_32d_ws2812b_64`, or `pio test -e wemos_d1_mini_ws2812b_64`. The tests cover valid state changes, malformed state rejection, and full-matrix payload validation; protocol commands also reject out-of-range brightness, coordinates, and RGB values at runtime.
