...
Run the parser unit tests on a connected board with `pio test -e waveshare_esp32_s3_matrix` or `pio test -e esp32_wroom_32d_ws2812b_64`. The test covers valid state changes and malformed state rejection; protocol commands also reject out-of-range brightness and RGB values at runtime.
